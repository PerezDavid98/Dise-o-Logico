...
def parse_factor(factor):
    if "d" in factor[0]:
        return dec_to_bin(factor[1:])[-8:]
    elif "h" in factor[0]:
        return hex_to_bin(factor[1:])[-8:]
    elif "b" in factor[0]:
        return factor[-8:]
    else:
        return dec_to_bin(factor)[-8:]
...
def binary_multiply(a, b):
    ...
    for j in range(min_len - 1, -1, -1):
        ...
        if len(temp) == 2:
            ...
            steps.append((temp[0], temp[1], product))
        elif len(temp) > 2:
            temp_index = len(temp)
            temp_index += 1
            product = binary_addition(str(product), str(temp[temp_index - 2]))
            steps.append((temp[temp_index - 2], product, product))
        else:
            pass

    steps.append((a, b, product[-8:]))

    return product[-8:], steps
...
if __name__ == '__main__':
    ...
    result, steps = binary_multiply(args.a, args.b)

    # Imprimir los pasos intermedios
    step_content = ""
    for step in steps:
        step_content += f"{step[0]} $\\times$ {step[1]} = {step[2]} \\\\ \n"

    # Imprimir el resultado en diferentes bases
    binario = int(result)
    decimal = int(result, 2)
    hexadecimal = hex(int(result, 2))
